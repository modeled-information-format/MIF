...
description: "Every published MIF release attaches SLSA build provenance and a CycloneDX SBOM to two reproducible artifacts, keyless via Sigstore OIDC, with publication fail-closed on in-run re-verification; the full quality-gate suite is wired to the org's central reusable workflows, SHA-pinned at ff8adc6b1267c272beef916af851d9506160354f."
type: adr
category: process
tags:
  - release
...
| In-run verify step re-checks SLSA provenance (`--predicate-type https://slsa.dev/provenance/v1`) and SBOM (`--predicate-type https://cyclonedx.org/bom`) for both artifacts before upload | `.github/workflows/release.yml` | L132-L153 | compliant |
...
attests each with SLSA build provenance and a CycloneDX SBOM keyless via
Sigstore, and gates publication on in-run re-verification. The quality-gate
