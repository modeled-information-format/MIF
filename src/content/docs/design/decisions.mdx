...
description: "Index of architectural decision records for the Modeled Information Format."
---
...
| ADR-008 | Decay Model Rationale | Accepted | Implements a configurable freshness/validity decay model (none, linear, exponential, step) with type-specific half-life defaults. The cognitive-memory rationale now lives in the AI Memory profile. |
...
MIF answers OKF's open "live vs stale" question with a configurable
freshness/validity decay model with four types (none, linear, exponential, step)
and type-specific default half-lives: P30D for semantic concepts, P7D for
episodic, and P14D for procedural. Strength is refreshed through access and
explicit reinforcement. This ADR is retained as a historical record; the
cognitive-memory rationale that originally motivated the half-life math now
lives in the AI Memory profile.
